...
export const InteractiveDiv = ({ image, title, handleShowModal, type }) => {
  ...
  return (
    <div
      ...
      } relative overflow-hidden border-[#B2A7B8] border-[1px] hover:border-[#fc398660] shadow-md shadow-black transition-all h-auto aspect-video
    ...
    >
      {isLoading && (
        <div className="w-full h-full bg-[rgba(0,0,0,0.55)] animate-pulse flex justify-center items-center gap-2">
          <div className="rounded-full w-4 h-4 bg-[#fc3986] animate-bounce delayed-1"></div>
          <div className="rounded-full w-4 h-4 bg-[#fc3986] animate-bounce delayed-2"></div>
          <div className="rounded-full w-4 h-4 bg-[#fc3986] animate-bounce delayed-3"></div>
        </div>
      )}
      <img
